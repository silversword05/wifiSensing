...
figure;
plot(t_test, pred, 'DisplayName', 'predicitions');
hold on;
plot(t_test, gt_test, 'DisplayName', 'ground-truth');
hold off;
legend;
...
x_train = train_dataset(:,1:end-2);
y_train = train_dataset(:,end);
x_test = test_dataset(:,1:end-2);
t_test = test_dataset(:, end-1);
